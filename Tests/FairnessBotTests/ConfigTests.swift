...
struct ConfigTests {
  ...
  @Test
  func `Fairness score threshold can be overridden`() throws {
    var env = baseValidEnv()
    env["FAIRNESS_SCORE_THRESHOLD"] = "80"

    let config = try Config(environment: env)
    #expect(config.fairnessScoreThreshold == 80)
  }

  @Test
  func `An invalid fairness score threshold throws`() {
    var env = baseValidEnv()
    env["FAIRNESS_SCORE_THRESHOLD"] = "not-a-number"
